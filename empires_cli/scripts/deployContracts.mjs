...
async function deployRealmsContracts(accountContracts, provider) {
  ...
  const compiled = json.parse(
    fs.readFileSync("compiled/Realms/ModuleController.json").toString("ascii")
  );
  ...
  console.log("address of the module controller contract: ", controllerAddress);
...
async function main() {
  ...
  const moduleAddresses = await deployRealmsContracts(userAddresses, provider);
